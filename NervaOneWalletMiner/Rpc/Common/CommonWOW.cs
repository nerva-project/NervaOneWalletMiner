    ...
    public static class CommonWOW
    {
        public static ServiceError GetServiceError(string source, dynamic error)
        {
            ...
            try
            {
                ...
                Logger.LogError("WOW.CGSE", source + " - error from service. Code: " + serviceError.Code + ", Message: " + serviceError.Message);
            }
            catch (Exception ex)
            {
                Logger.LogException("WOW.CGSE", ex);
            }
        ...
        {
            return Math.Round(Convert.ToDecimal(balanceAtomic / 100000000000.0), decimalPlaces);
        }

        public static ulong AtomicUnitsFromDoubleAmount(decimal amount)
        {
            return (ulong)(amount * Convert.ToDecimal(100000000000.0));
        }
