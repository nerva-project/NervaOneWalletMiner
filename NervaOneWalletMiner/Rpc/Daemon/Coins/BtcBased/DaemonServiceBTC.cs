...
namespace NervaOneWalletMiner.Rpc.Daemon
{
    ...
    internal class DaemonServiceBTC : DaemonServiceBaseBTC
    {
        ...
        protected override double BlockSeconds => 600.0;
        ...
        public override async Task<GetInfoResponse> GetInfo(RpcBase rpc, GetInfoRequest requestObj)
        {
            ...
            try
            {
                ...
                if (httpResponse.IsSuccessStatusCode)
                {
                    ...
                    {
                        Logger.LogInfo("BTC.DGTI", "Response Content is empty");
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    responseObj.NetworkHashRate = getInfoResponse.difficulty * (4294967296.0 / BlockSeconds);

                                    /*
                                     * difficulty = hashrate / (2^256 / max_target / intended_time_per_block)
                                     * = hashrate / (2^256 / 2^224 / 600)
                                     * = hashrate / (2^32 / 600)
                                     * = hashrate / 7158278.83
                                     */
                                }
            ...
            {
                Logger.LogException("BTC.DGTI", ex);
            }
